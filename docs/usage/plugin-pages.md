...
可靠投递批次按 `batch_id` 合并为一个逻辑展示单元，页面会在批次标题下按 `output_order` 展示完整的 card / standard 输出。分页按逻辑单元而不是历史行数计算：一个批次有 21 条输出时，即使页面大小为 20，也会在同一页完整展示 21 条，不会在下一页再次出现同一个批次；顶部记录数仍统计实际历史行数。

批次仍处于 `pending`、`waiting`、失败或其他未解决状态时，批次标题会显示未完成提示，批次输出的单条删除按钮保持禁用；应使用「重试」或「丢弃批次」推进状态。批次状态会跨页随每条输出返回，避免状态筛选后误解除保护。
