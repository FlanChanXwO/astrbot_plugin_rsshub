...
**可配置项：** `proxy`/`rsshub_base_url`/`default_interval`/`minimal_interval`/`timeout`/`download_image_before_send`/`failed_queue_capacity`/`sender_strategy_telegram`/`sender_strategy_aiocqhttp`/`deduplicate_multi_bot`/`platform_shared_data_aiocqhttp`
