...
- Telegram local image files larger than the built-in photo threshold (default 10 MiB) must be sent as files instead of photos to avoid Bot API photo size rejection.
...
- Media sending always pre-downloads media before building platform components. Only successful media downloads are cached; validate local images/videos before caching or reusing them, do not reintroduce in-memory or disk failure caches, and do not restore a user-facing `download_media_before_send` toggle.
- Platform media limits and defaults belong in `src/shared/constants.py`; do not expose cache GC, media integrity thresholds, or platform degradation strategy as user config unless there is a clear environment-specific reason.
- OneBot defaults to local video files after successful pre-download; `prefer_local_video=false` is only a compatibility override.
...
- `_conf_schema.json` only exposes startup-level config, HTTP network config, Routes KB provider/source settings, content handler AI provider/persona selection, credentials, and platform strategies; subscription defaults belong in Plugin Pages.
...
- Shared cross-layer constants live in `src/shared/constants.py`; do not recreate `src/domain/constants.py`. Typed config/runtime models live under `src/infrastructure/config/models/`: persisted Pydantic config models in `plugin_config_models.py`, runtime dataclass settings in `runtime_settings.py`, and sender strategy config compatibility in `sender_strategy_models.py`. Config parsing lives under `src/infrastructure/config/`: legacy config migration in `legacy_migration.py`, schema self-heal in `schema_healer.py`, runtime settings assembly in `settings_builder.py`, `config_loader.py` as the public facade, and `datamodels.py` only as compatibility re-exports.
